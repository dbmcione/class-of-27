...
/**
 * The caption for a "Challenge a friend" tap before a round exists to show a
 * score for — from the transition screen, say, rather than the score page.
 * No `ref` to attach yet: that code is minted per finished round, and there
 * isn't one at this point in the flow.
 */
export function buildInviteCaption(): string {
  return [
    `Hey, I'm playing Spot the Diagnosis by DBMCI One — can you spot the ` +
      `diagnosis before I do? Try it:`,
    '',
    gameLink(),
  ].join('\n');
}
...
/**
 * Text-only, unlike `shareScorecard` — there is no image to attach yet, so
 * this shares the plain invite caption, falling back to the clipboard rather
 * than a file download when the platform has no share sheet.
 */
export async function shareInvite(): Promise<InviteShareOutcome> {
  const text = buildInviteCaption();

  if (typeof navigator.share === 'function') {
    try {
      await navigator.share({ title: CHALLENGE_NAME, text });
      return { kind: 'shared' };
    } catch (error) {
      if (error instanceof DOMException && error.name === 'AbortError') {
        return { kind: 'cancelled' };
...
export function inviteShareHint(outcome: InviteShareOutcome): string {
  switch (outcome.kind) {
    case 'copied':
      return 'Link copied — paste it to a friend.';
    case 'failed':
      return 'Couldn’t share right now.';
    case 'shared':
    case 'cancelled':
      return '';
  }
